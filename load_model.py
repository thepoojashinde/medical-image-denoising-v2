...
def evaluate_model(model, X, Y):
    ...
    for i in range(len(X)):
        ...
        if i < 3:  # show only a few for brevity
...
def predict_on_new_images(model, paths):
    for idx, path in enumerate(paths):
        try:
            ...
            mse_val = mean_squared_error(clean_img[:, :, 0], denoised_img[:, :, 0])
            ssim_val = ssim(clean_img[:, :, 0], denoised_img[:, :, 0], data_range=1.0)
            psnr_val = psnr(clean_img[:, :, 0], denoised_img[:, :, 0], data_range=1.0)
            ...
            plt.subplot(1, 3, 2); plt.title("Denoised"); plt.imshow(denoised_img[:, :, 0], cmap='gray'); plt.axis('off')
            plt.subplot(1, 3, 3); plt.title("Clean"); plt.imshow(clean_img[:, :, 0], cmap='gray'); plt.axis('off')
